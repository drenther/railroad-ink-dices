...
export const Dice = forwardRef(
  // @ts-ignore
  (props: TProps, ref: React.MutableRefObject<TDiceRef>) => {
    const {
      rollingTime = 1000,
      onRoll,
      defaultValue = 6,
    ...
    const seeded = React.useRef(random.createRandom(seed ?? 0));
    const handleDiceRoll = (value?: TValue) => {
      ...
      setTimeout(() => {
        if (seeded.current) {
          let rollValue = seeded.current.rangeFloor(1, 6) as TValue;
